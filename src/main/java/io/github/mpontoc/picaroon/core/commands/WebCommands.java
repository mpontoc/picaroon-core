...
public class WebCommands {
	...
	public static void swichToLastWindow() {

		ArrayList<String> windows = new ArrayList<String>(driver.getWindowHandles());
		Log.log(windows.toString());
		if (windows.size() > 1) {
			Log.log(windows.get(windows.size() - 1));
			driver.switchTo().window((String) windows.get(windows.size() - 1));
		}
	}
	...
	public static void swichToLastWindowClosePrevious() {

		ArrayList<String> windows = new ArrayList<String>(driver.getWindowHandles());
		Log.log(windows.toString());
		if (windows.size() > 1) {
			for (int i = 0; i < windows.size() - 1; i++) {
				Log.log(windows.get(i));
